...
def main():
    ...
    parser_prep.add_argument('--retro-anno', type=Path, metavar='PATH', help='Path of the retroelement annotation GTF file')
    ...
    parser_extract.add_argument('--ref-seq', type=Path, required=True, metavar='PATH', help='Path of the genome reference fasta file')
...
def prepare(args):
    ...
    retro_anno = args.retro_anno
    assert retro_anno.exists()
    ...
    if not args.dry_run:
        ...
        (out_dir / 'retroelements').mkdir(exist_ok=True)
        (out_dir / 'ATS_APA').mkdir(exist_ok=True)
    ...
    retro_gtf = ref_dir / 'retroelements.gtf'
    if not retro_gtf.exists() or args.rerun_all:
        message('Copying retroelement annotation GTF file')
        if not args.dry_run:
            subprocess.run(['cp', str(retro_anno), str(retro_gtf)], check=True)
    chr_len = ref_dir / 'chr_lengths.genome'
    mc_gtf = ref_dir / 'transcripts.gtf'
    if not (chr_len.exists() and mc_gtf.exists()) or args.rerun_all:
        message('Creating reference files for mountainClimber')
        if not args.dry_run:
            script = Path(__file__).parent.parent / 'src' / 'prep_mountainClimber.sh'
            subprocess.run(['sh', script, ref_seq, ref_anno, chr_len, mc_gtf])
...
def extract(args):
    ...
    retro_dir = out_dir / 'retroelements'
    atsapa_dir = out_dir / 'ATS_APA'
    ...
    retro = retro_dir / f'{args.id}-telescope_report.tsv'
    if not retro.exists() or args.rerun_all:
        message('Counting retroelement expression')
        retro_gtf = ref_dir / 'retroelements.gtf'
        if not args.dry_run:
            run_telescope(bam, retro_gtf, retro_dir, args.id)

    atsapa = atsapa_dir / f'{args.id}.ru.bed'
    if not atsapa.exists() or args.rurun_all:
        message('Counting alternative TSS and TES')
        if not args.dry_run:
            run_mountainClimber(bam, args.ref_seq)
...
def combine(args):
    ...
    assemble_retroelement_bed(samples, retro_dir, retro_gtf, out_dir)
...
def run_telescope(bam: Path, retro_gtf: Path, retro_dir: Path, sample_id: str):
...
def run_mountainClimber(bam: Path):
    # SAMPLE=$1
    # REF_SEQ=$2
    # CODE_DIR=$3
    # OUT_DIR=$4
...
def load_retros(retro_gtf: Path) -> pd.DataFrame:
...
def assemble_retroelement_bed(samples: list, retro_dir: Path, retro_gtf: Path, out_dir: Path):
    ...
    anno = load_retros(retro_gtf)
    df.index = df.index.rename('name')
    df = anno.merge(df.reset_index(), on='name', how='inner')
    df.to_csv(out_dir / 'retroelements.bed', sep='\t', index=False, float_format='%g')
